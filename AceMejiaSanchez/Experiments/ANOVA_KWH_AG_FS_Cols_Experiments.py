...
for i in range(150):  
    ...
    n = 10
    prob = .5
    binomial_150 = np.random.binomial(n, prob, 150)

    # Poisson column
    lamda = 1
    poisson_150 = np.random.poisson(lamda, 150)

    # Gaussian column
    mu = 100
    sigma = 50
    gaussian_150 = np.random.normal(mu, sigma, 150)

    # Uniform column
    high = 1
    low = 0
    uniform_150 = np.random.uniform(low, high, 150)
...
for i in range(150):  
    ...
    if p3 < 0.05:
        ...
        FS_scatterplot_SIG_pois_gaus.append({"RUN #": i, 
                                "Poisson 150": poisson_150, 
                                "Uniform 150": uniform_150})
    else:
        FS_pois_gaus_sigs.append(False)
        FS_scatterplot_NO_SIG_pois_gaus.append({"RUN #": i, 
                                "Poisson 150": poisson_150, 
                                "Uniform 150": gaussian_150})
    
    if p4 < 0.05:
        FS_pois_uni_sigs.append(True)
        FS_scatterplot_SIG_pois_uniform.append({"RUN #": i, 
                                "Poisson 150": poisson_150, 
                                "Uniform 150": uniform_150})
    else:
        FS_pois_uni_sigs.append(False)
        FS_scatterplot_NO_SIG_pois_uniform.append({"RUN #": i, 
                                "Poisson 150": poisson_150, 
                                "Uniform 150": uniform_150})
